...
function CompaniesPage({ companies, filters, onFilters, onRefresh, onSelect }) {
  return <div className="admin-page">
    <div className="admin-page-intro"><div><h2>Companies</h2><p>Find workspaces, see who uses them, and open company administration or Support Mode.</p></div></div>
    <div className="admin-toolbar"><input aria-label="Search companies" placeholder="Search companies..." value={filters.search} onChange={(event) => onFilters({ ...filters, search: event.target.value })} /><select aria-label="Filter by status" value={filters.status} onChange={(event) => onFilters({ ...filters, status: event.target.value })}><option value="">All statuses</option>{SUBSCRIPTION_STATUS_OPTIONS.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}</select><select aria-label="Filter by plan" value={filters.plan} onChange={(event) => onFilters({ ...filters, plan: event.target.value })}><option value="">All plans</option><option value="basic">Basic</option><option value="plus">Plus</option><option value="pro">Pro</option></select><button className="admin-secondary" onClick={onRefresh} type="button">Apply filters</button></div>
    <AdminTable empty="No companies match these filters."><table className="admin-table"><thead><tr><th>Company</th><th>Country</th><th>Plan</th><th>Status</th><th>Users</th><th>Created</th><th>Open</th></tr></thead><tbody>{companies.map((company) => <tr key={company.id}><td><strong>{company.trading_name || company.name}</strong><small>{company.name}</small></td><td>{company.country || 'Unknown'}</td><td>{company.plan_name}</td><td><AdminStatus status={company.subscription_status} /></td><td>{company.customer_user_count}</td><td>{formatAdminDate(company.created_at)}</td><td><button className="admin-secondary small" onClick={() => onSelect(company.id)} type="button">Company detail</button></td></tr>)}</tbody></table></AdminTable>
  </div>;
}

function SubscriptionsPage({ companies, filters, onFilters, onRefresh, onSelect }) {
  return <div className="admin-page">
    <div className="admin-page-intro"><div><h2>Subscriptions</h2><p>Manage plan access, trial dates, and subscription status. Changes are permission protected and audit logged.</p></div></div>
    <div className="admin-toolbar"><input aria-label="Search subscriptions" placeholder="Search companies..." value={filters.search} onChange={(event) => onFilters({ ...filters, search: event.target.value })} /><select aria-label="Filter subscriptions by status" value={filters.status} onChange={(event) => onFilters({ ...filters, status: event.target.value })}><option value="">All access states</option>{SUBSCRIPTION_STATUS_OPTIONS.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}</select><select aria-label="Filter subscriptions by plan" value={filters.plan} onChange={(event) => onFilters({ ...filters, plan: event.target.value })}><option value="">All plans</option><option value="basic">Basic</option><option value="plus">Plus</option><option value="pro">Pro</option></select><button className="admin-secondary" onClick={onRefresh} type="button">Apply filters</button></div>
    <AdminTable empty="No subscriptions match these filters."><table className="admin-table admin-subscriptions-table"><thead><tr><th>Company</th><th>Plan</th><th>Status</th><th>Trial started</th><th>Trial ends</th><th>Days remaining</th><th>Access</th><th>Manage</th></tr></thead><tbody>{companies.map((company) => <tr key={company.id}><td><strong>{company.trading_name || company.name}</strong><small>{company.name}</small></td><td>{company.plan_name}</td><td><AdminStatus status={company.subscription_status} /></td><td>{formatAdminDate(company.trial_started_at, '—')}</td><td>{formatAdminDate(company.trial_ends_at, '—')}</td><td>{subscriptionDaysRemaining(company)}</td><td><span className={`admin-access-state admin-access-${company.subscription_status}`}>{['active', 'trialing'].includes(company.subscription_status) ? 'Access open' : 'Access restricted'}</span></td><td><button className="admin-secondary small" onClick={() => onSelect(company.id)} type="button">Manage access</button></td></tr>)}</tbody></table></AdminTable>
  </div>;
}

function subscriptionDaysRemaining(company) {
  if (company.subscription_status !== 'trialing' || !company.trial_ends_at) return '—';
  const days = Math.max(0, Math.ceil((new Date(company.trial_ends_at).getTime() - Date.now()) / 86400000));
