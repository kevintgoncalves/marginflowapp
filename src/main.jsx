...
function App({ authMembership, authUser, demoMode = false, onSignOut }) {
  ...
  const diagnoseCurrentLaptopRecovery = async () => {
    ...
    }, { exampleLimit: 15, baselineRelationalCount: 346 });
  };
        ...
        {recoveryConflictDiagnostic && (
          <div className="recovery-preview recovery-diagnostic">
            ...
            <div className="recovery-summary-grid recovery-catalog-grid">
              ...
              <div><strong>Pre-flagged conflict provenance</strong><span>Flagged {recoveryConflictDiagnostic.preFlaggedConflictAnalysis?.totalPreFlagged || 0} · No candidate {recoveryConflictDiagnostic.preFlaggedConflictAnalysis?.noRelationalCandidate || 0} · Multiple {recoveryConflictDiagnostic.preFlaggedConflictAnalysis?.multipleCandidates || 0}</span><span>Equivalent {recoveryConflictDiagnostic.preFlaggedConflictAnalysis?.exactlyOneMateriallyEquivalentCandidate || 0} · Different {recoveryConflictDiagnostic.preFlaggedConflictAnalysis?.exactlyOneMateriallyDifferentCandidate || 0} · Stale estimate {recoveryConflictDiagnostic.preFlaggedConflictAnalysis?.staleAgainstCurrentRelational || 0}</span></div>
              ...
              <div><strong>Document number quality</strong><span>{recoveryConflictDiagnostic.documentNumberQuality?.suspiciousValues?.length || 0} suspicious value(s) · {recoveryConflictDiagnostic.documentNumberQuality?.repeatedValues?.length || 0} repeated value(s)</span><span>{recoveryConflictDiagnostic.documentNumberQuality?.suspiciousValues?.[0] ? `${recoveryConflictDiagnostic.documentNumberQuality.suspiciousValues[0].value}: ${recoveryConflictDiagnostic.documentNumberQuality.suspiciousValues[0].count}` : "No generic placeholders detected"}</span></div>
              <div><strong>Relational growth audit</strong><span>Baseline {recoveryConflictDiagnostic.relationalGrowthAudit?.baselineCount ?? "Unknown"} · Current {recoveryConflictDiagnostic.relationalGrowthAudit?.currentCount ?? "Unknown"} · Delta {recoveryConflictDiagnostic.relationalGrowthAudit?.delta ?? "Unknown"}</span><span>{recoveryConflictDiagnostic.relationalGrowthAudit?.latestCreatedCandidates?.length || 0} latest creation candidate(s) included in export</span></div>
            </div>
